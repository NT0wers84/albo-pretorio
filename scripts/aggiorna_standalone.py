...
def _ft_link(url: str, testo: str) -> str:
    """Link per il footer del sito (raw-encoded per il bundle JSON)."""
    return (
...
# PATCH 8 — Footer completo: "Come funziona" + gemello + Feed RSS
# Idempotency: se "Come funziona" è già nel raw, saltiamo.
# Âncora di ricerca: il commento HTML <!-- FOOTER --> sempre presente nel bundle.
...
PATCH8_FOOTER_NEW = (
    ...
    f'<strong>Come funziona.<\\/strong>'
    f" Un automatismo legge ogni giorno l'Albo Pretorio del Comune di"
    f" Pieve Emanuele, scarica gli atti e li riassume con l'intelligenza"
    f" artificiale. L'estrazione automatica può contenere errori:"
    f" fa fede sempre l'atto originale, linkato in ogni scheda."
    f'<\\/div>\\n'
...
# PATCH 9 — Header: allinea il colore sfondo a bilanciopertutti (#123785)
...
def applica_patch_raw(raw: str) -> str:
    ...
    if "rias-wrap" in raw:
        ...
        if m_p1:
            raw = raw[:m_p1.start()] + PATCH1_NEW + raw[m_p1.end():]  # noqa: E501
    ...
    # "sc-if" compare solo in PATCH2_NEW, non in PATCH2_OLD
    if "sc-if" in raw:
    ...
    if _PATCH8_DONE in raw:
        ...
        fi_end   = raw.find(_PATCH8_FT_END, fi_start) + len(_PATCH8_FT_END)
    ...
    # PATCH 9 — Header: sfondo #123785 (allineato a bilanciopertutti)
